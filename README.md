...
- `/help`는 버튼 메뉴 표시
- `세션`, `모델`, `Low (Fast)`, `Thinking`, `권한`, `최근 출력`, `현재 상태`, `취소` 버튼 지원
- `Low (Fast)`는 `model_reasoning_effort=low` 별칭이며, 토큰 생성 속도 2배를 보장하는 모드는 아님
- 세션 메뉴에서 `디렉토리 설정` 버튼으로 현재 `/resume` 기준 폴더를 바꿀 수 있음
- `/resume`은 현재 디렉토리와 하위 폴더 세션만 보여주며, 디렉토리 브라우저에서 세션 미리보기를 같이 보여줌
