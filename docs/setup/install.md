...
6. 이후 세션 메뉴는 그 폴더에서 터미널로 `codex`를 실행한 뒤 `/resume` 했을 때와 같은 `동일 cwd` 세션만 보여줍니다.
7. 세션 메뉴에서 보이는 세션을 선택하거나, 필요하면 `새 세션 만들기`
8. 새 세션을 만들 때는 폴더를 눌러 이동하거나 `../`로 상위 폴더 이동
9. `현재 폴더로 세션 시작` 또는 `새 폴더 만들기`
10. `새 폴더 만들기`를 눌렀다면 다음 일반 메시지로 폴더 이름 전송
11. `/help`
12. `모델`
13. 모델 선택
14. 자동으로 뜬 `Thinking` 메뉴에서 선택
15. `/help`
16. 필요하면 `Fast`를 눌러 `fast_mode`를 켜거나 끔
17. `권한`
18. `Full` 선택
19. 평문으로 작업 지시 전송
